...
export default function Login() {
  ...
  async function handleGoogleMock() {
    setLoading(true);
    const result = await login('admin@gov.in', 'password');
    setLoading(false);
    if (result.ok) navigate('/');
    else setError(result.error);
  }
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', minHeight:'100vh', background:'var(--bg-page)' }}>
      <div style={{ width:'100%', maxWidth:'420px', padding:'40px', background:'var(--bg-surface)', border:'1px solid var(--border)', borderRadius:'var(--r-lg)', boxShadow:'var(--shadow-lg)' }}>
        ...
        <button type="button" onClick={handleGoogleMock} style={{ width:'100%', padding:'10px', background:'var(--bg-surface)', color:'var(--text-primary)', border:'1px solid var(--border)', borderRadius:'var(--r-md)', fontSize:'0.9rem', fontWeight:500, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'10px', transition:'background var(--transition)', fontFamily:'inherit' }}>
